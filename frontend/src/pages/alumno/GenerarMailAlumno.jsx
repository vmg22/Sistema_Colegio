...
import { api } from "../../api/fetchConfig"; // Corregido: Importar api en lugar de axios
...
// API específica para mail (Ya no se usa BASE_URL explícito porque api lo maneja)
const MAIL_ENDPOINT_PREFIX = "/mail";
...
const GenerarMailAlumno = () => {
  ...
  const construirRequestBody = () => {
    ...
    const requestBody = {
      ...
      dni: reporte.dni,
      anio: reporte.anio_lectivo || new Date().getFullYear(),
    };
  ...
  const handleEnviarMail = async () => {
    ...
    try {
      ...
      const data = await api.post(
        `${MAIL_ENDPOINT_PREFIX}${opcion.endpoint}`,
