...
// [DUPLICADOS ELIMINADOS - YA DEFINIDOS ARRIBA]
// const nodemailer = require("nodemailer");
// const dotenv = require("dotenv");
// const pool = require("../config/db");
// ... imports eliminados ...
...
const obtenerDatosAlumno = async (dni, anio) => {
  try {
    console.log(`🔍 obtenerDatosAlumno - Buscando DNI: ${dni}, Año: ${anio}`); // DEBUG
    const [rows] = await pool.execute(QUERY_ALUMNO_BASICO, [dni, anio]);

    if (rows.length === 0) {
      throw new Error(`Alumno no encontrado o sin datos para ese año lectivo. (DNI: ${dni}, Año: ${anio})`);
    }
...
                .warning-icon { font-size: 48px; text-align: center; margin: 10px 0; }
                .progress-bar { width: 100%; height: 30px; background-color: #e0e0e0; border-radius: 15px; overflow: hidden; margin: 10px 0; }
                .progress-fill { height: 100%; background-color: ${datos.asistenciasPorcentaje >= 90 ? "#ff9800" : "#f44336"
    }; text-align: center; line-height: 30px; color: white; font-weight: bold; }
            </style>
        </head>
        <body>
...
                    <div class="warning-icon">⚠️</div>
                    <div class="alert-box">
                        <strong>Atención:</strong> El alumno está próximo a alcanzar el límite de faltas permitidas (${datos.asistenciasPorcentaje
    }% de asistencia).
                    </div>
                    
                    <h2>Información del Alumno</h2>
...
                        <tr>
                            <td>Alumno:</td>
                            <td>${datos.nombreAlumno} ${datos.apellidoAlumno
    }</td>
                        </tr>
                        <tr>
                            <td>Curso:</td>
...
                    <h3>Resumen de Asistencias</h3>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: ${datos.asistenciasPorcentaje
    }%;">
                            ${datos.asistenciasPorcentaje}%
                        </div>
                    </div>
...
                        <tr>
                            <td>Clases Presente:</td>
                            <td style="color: #4CAF50;"><strong>${datos.clasesPresente
    }</strong></td>
                        </tr>
                        <tr>
                            <td>Faltas Actuales:</td>
                            <td style="color: #f44336;"><strong>${datos.faltasActuales
    }</strong></td>
                        </tr>
                        <tr>
                            <td>Faltas Máximas:</td>
...
                        <tr>
                            <td>Faltas Restantes:</td>
                            <td><strong style="color: #d32f2f;">${datos.faltasMaximas - datos.faltasActuales
    }</strong></td>
                        </tr>
                    </table>
                    
...
                    <p>Estimado/a ${datos.nombreTutor},</p>
                    
                    <p>Por medio del presente, le convocamos a una reunión referente al alumno <strong>${datos.nombreAlumno
    }</strong> del curso <strong>${datos.curso}</strong>.</p>
                    
                    <div class="reunion-box">
                        <strong>Motivo:</strong> ${datos.motivo}
