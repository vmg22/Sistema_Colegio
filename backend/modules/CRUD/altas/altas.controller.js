...
const altaController = {
  ...
  crearUsuarioParaDocente: async (req, res) => {
    try {
      ...
      const usuarioVinculado = await altaServices.crearUsuarioParaDocente(
        id,
        datosUsuario
      );
      exito(
        res,
        "Usuario creado y vinculado exitosamente",
        usuarioVinculado,
        201
      );
    } catch (err) {
      ...
      if (err.code === "ER_DUP_ENTRY") {
        return error(
          res,
          "El Username o Email ya están en uso",
          409,
          err.message
        );
      }
  ...
  actualizarDocente: async (req, res) => {
    try {
      ...
      const docenteActualizado = await altaServices.actualizarDocente(
        id,
        datosActualizacion
      );
  ...
  actualizarDocenteParcial: async (req, res) => {
    try {
      ...
      // Apunta a la nueva función de servicio para PATCH
      const docenteActualizado = await altaServices.actualizarDocenteParcial(
        id,
        datosActualizacion
      );
      ...
      if (err.message.includes("DNI ya está registrado")) {
        return error(res, err.message, 409);
      }
      error(res, "Error al actualizar docente", 500, err.message);
    }
